...
| 類別 | 限制項目 | 上限 | 超過上限時系統行為 | 提示訊息（toast） |
|---|---|---:|---|---|
| 機台設定 | 機台種類數（Machine Types） | 10 種 | 禁止新增第 11 種 | 機台種類數已達上限（10） |
| 機台設定 | 每種機台台數 count | 10 台 | 輸入值自動修正回 10（spinner 也無法超過） | 每種機台最多 10 台 |
| 工件設定 | 工件/產品種類數（Products） | 10 個 | 禁止新增第 11 個 | 產品數已達上限 |
| 工件設定 | 每個工件流程步驟（Route Steps） | 10 步 | 禁止新增第 11 步 | 流程步驟已達上限（10） |
| 工件設定 | 每個工件數量 quantity | 5 個 | 輸入值自動修正回 5（spinner 也無法超過） | 每個工件數量最多 5 |
